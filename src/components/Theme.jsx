...
function Theme({ theme, setTheme, containerColor, buttonColor }) {
  ...
  const ThemeValue = [
    ...
    { name: "kobayashi", color: "bg-[#ebcd01] text-[#f01200]" },
  ...
  return (
    <>
      ...
      <div className={`${Open ? "OpenMenu" : "CloseMenu"} shadow-lm w-[400px] h-full ${containerColor} absolute left-0 p-4 overflow-x-auto thin-scrollbar`}>
        {ThemeValue.map((item, index) => (
          <div
            key={index}
            onClick={() => handleThemeChange(item.name)}
            className={`${item.color} w-full h-[130px] rounded ${item.name !== "blue" && "mb-[27px]"} text-4xl flex justify-center items-center hover:scale-102 active:scale-95 transition-all duration-100 ease-in-out ScrollAnimation ${theme === item.name ? "ring-4 ring-orange-500" : ""}`}
          >
            <h1>
              {item.name === "dots" ? (
                <>
