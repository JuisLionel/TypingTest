...
function Results({ correctChars, wrongChars, correctWords, time, onRestart, TopSpeed, setTopSpeed, openResult, wordCount }) {
      ...
      <div className={`flex w-[900px] h-[500px] bg-blue-900 rounded p-4 text-white gap-6 ${openResult ? "Open" : "Close"}`}>
        <div className="flex flex-col justify-start gap-4 w-[250px]">
          ...
          <div className="bg-blue-700 p-4 rounded hover:scale-102 transition-all ease-linear duration-100">
            ...
            <p className="text-2xl font-bold">{correctWords}/{wordCount}</p>
          </div>

          <button
            onClick={onRestart}
            className="mt-7 bg-blue-700 hover:scale-102 active:scale-98 transition-all ease-linear duration-100 px-4 py-2 rounded text-white"
          >
            Restart
          </button>
        </div>
